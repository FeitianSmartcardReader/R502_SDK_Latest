...
######
	echo
	echo "Welcome To R502 For Linux"
	echo

#modify ccid info.plist  R502
	if [ -f /usr/lib/pcsc/drivers/ifd-ccid.bundle/Contents/Info.plist ]; then
		declare -i start=0
	       	declare -i end=0
	        declare -i headline=0
	        declare -i endline=0
		start=`cat -n /usr/lib/pcsc/drivers/ifd-ccid.bundle/Contents/Info.plist | grep "FT R502" | head -1 | cut -f1`
	        if [ $start = 0 ]; then
			...
			cp Info.plist Info.plist.bak
	                ...
	                if [ $? = 0 ]; then
                		...
                		endline=$end-$start-1
                		head -n $headline Info.plist.bak > Info.plist
				echo	"                <string>0x0608</string>"   >> Info.plist
                		tail -n $endline Info.plist.bak			    >> Info.plist
        		else
				echo "error: edit Info.plist"
			fi
			...
			cp Info.plist Info.plist.bak
                	...
                	if [ $? = 0 ]; then
                		...
                		endline=$end-$start-1
                		head -n $headline Info.plist.bak > Info.plist
				echo	"                <string>FT R502 CL</string>"	>> Info.plist
                		tail -n $endline Info.plist.bak					>> Info.plist
        		else
				echo "error: edit Info.plist"
			fi
...
#modify ccid info.plist  R502-DUAL
	if [ -f /usr/lib/pcsc/drivers/ifd-ccid.bundle/Contents/Info.plist ]; then
		declare -i start=0
	       	declare -i end=0
	        declare -i headline=0
		...
		start=`cat -n /usr/lib/pcsc/drivers/ifd-ccid.bundle/Contents/Info.plist | grep "FT R502" | head -1 | cut -f1`
			...
			cp Info.plist Info.plist.bak
	                ...
	                if [ $? = 0 ]; then
                		...
                		endline=$end-$start-1
                		head -n $headline Info.plist.bak > Info.plist
				echo	"                <string>0x060D</string>"   >> Info.plist
                		tail -n $endline Info.plist.bak			    >> Info.plist
        		else
				echo "error: edit Info.plist"
			fi
			...
			cp Info.plist Info.plist.bak
                	...
                	if [ $? = 0 ]; then
                		...
                		endline=$end-$start-1
                		head -n $headline Info.plist.bak > Info.plist
				echo	"                <string>FT R502 DUAL</string>"	>> Info.plist
                		tail -n $endline Info.plist.bak					>> Info.plist
        		else
				echo "error: edit Info.plist"
			fi
...
######
	echo
	echo "Welcome To R502-U For Mac OS X"
	echo

#modify ccid info.plist  R502-U
	if [ -f /usr/libexec/SmartCardServices/drivers/ifd-ccid.bundle/Contents/Info.plist ]; then
		declare -i start=0
		declare -i end=0
		declare -i headline=0
		declare -i endline=0
		start=`cat -n /usr/libexec/SmartCardServices/drivers/ifd-ccid.bundle/Contents/Info.plist | grep "FT R502-U" | head -1 | cut -f1`
		if [ $start = 0 ]; then
			...
			cp Info.plist Info.plist.bak
		        ...
		        if [ $? = 0 ]; then
		        	...
		        	endline=$end-$start-1
		        	head -n $headline Info.plist.bak > Info.plist
				echo	"                <string>FT R502</string>"	>> Info.plist
		        	tail -n $endline Info.plist.bak					>> Info.plist
			else
				echo "error: edit Info.plist"
			fi
